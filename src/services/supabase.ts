...
// Game Session related functions - ONLY FOR REGISTERED USERS
export async function createGameSession(sessionData: {
  ...
  if (currentUser?.is_guest || currentUser?.username === 'GST') {
    console.log('👤 Guest user detected - skipping game session creation');
    return null;
  }
...
export async function updateGameSession(sessionId: string, updates: {
  ...
  const currentUser = await getCurrentUser();
  if (currentUser?.is_guest || currentUser?.username === 'GST') {
    console.log('👤 Guest user detected - skipping game session update');
    return null;
  }
...
// Enhanced task functions for game sessions - ONLY FOR REGISTERED USERS
export async function addTaskToSession(task: {
  ...
  const currentUser = await getCurrentUser();
  if (currentUser?.is_guest || currentUser?.username === 'GST') {
    console.log('👤 Guest user detected - skipping task creation');
    return null;
  }
...
export async function updateTaskStatus(taskId: string, updates: {
  ...
  const currentUser = await getCurrentUser();
  if (currentUser?.is_guest || currentUser?.username === 'GST') {
    console.log('👤 Guest user detected - skipping task status update');
    return null;
  }
...
export async function updateUserStats(userId: string, updates: {
  ...
  const currentUser = await getCurrentUser();
  if (currentUser?.is_guest || currentUser?.username === 'GST') {
    console.log('👤 Guest user detected - skipping user stats update');
    return null;
  }
...
// Leaderboard related functions - ONLY FOR REGISTERED USERS
...
export async function updateLeaderboard(entry: { user_id: string; username: string; score: number }) {
  ...
  const currentUser = await getCurrentUser();
  if (currentUser?.is_guest || currentUser?.username === 'GST') {
    console.log('👤 Guest user detected - skipping leaderboard update');
    return null;
  }
