...
const QuickBattle: React.FC = () => {
  ...
  function addTask(){
    ...
    setTitle(''); setMins(remain-mins>=5?mins:Math.max(remain,5));
    setTimeout(()=>inputRef.current?.focus(),0);
  }
  const del = (id:string)=>setTasks(x=>x.filter(t=>t.id!==id));

  /* DnD ----------------------------------------------------------- */
  const dragStart=(e:React.DragEvent,id:string)=>{setDrag(id);e.dataTransfer.effectAllowed='move';};
  const onOver=(e:React.DragEvent,ix:number)=>{e.preventDefault();setHover(ix);};
  const onDrop=(e:React.DragEvent,ix:number)=>{
    e.preventDefault(); if(!drag) return;
    const from=tasks.findIndex(t=>t.id===drag); if(from===ix)return;
    const arr=[...tasks]; const [item]=arr.splice(from,1); arr.splice(ix,0,item);
    setTasks(arr); setDrag(null); setHover(null);
  };

  const start=()=>ready&&nav('/fighter-select',{state:{tasks,breakMinutes:breakM}});

  /* ------------------------------ render ------------------------- */
  return (
    <main className="min-h-screen bg-bezel text-white font-arcade flex justify-center py-10 px-4">
      {/* grid: row1 title – row2 content */}
      <div className="grid grid-rows-[auto_1fr] grid-cols-[minmax(0,840px)_minmax(260px,340px)] gap-x-14 gap-y-10 w-full max-w-7xl">
        ...
        <section className="row-start-2 space-y-10">
          ...
          <div className="bg-crtBlue/40 border-4 border-crtBlue rounded-lg p-6 space-y-4">
            ...
            <div className="flex gap-3 items-stretch">
              ...
              <select
                ...
                className="w-[6rem] bg-bezel border-2 border-crtBlue text-sm pl-2 focus:border-primary outline-none"
                disabled={remain<=0}
              >
          ...
          {/* mobile start */}
          <Start ready={ready} total={total} start={start} className="xl:hidden"/>
        </section>
        ...
        <div className="row-start-2 flex flex-col gap-8">
          <Review
            ...
            del={del}
          />
          <Start ready={ready} total={total} start={start}/>
        </div>
      </div>
    </main>
  );
  ...
  <div className="bg-bezel/50 border-4 border-crtBlue rounded-lg p-5 space-y-4 w-full">
    ...
    <ul className="space-y-2 max-h-[28rem] overflow-y-auto pr-1">
      ...
      <li onDragOver={e=>dragOver(e,tasks.length)} onDrop={e=>drop(e,tasks.length)} className="h-4">
        {hover===tasks.length && <div className="h-1 bg-neonYel animate-pulse rounded"/>}
      </li>
    </ul>
  </div>
);
...
/* -------- Start button ------------------------------------------ */
const Start:React.FC<{ready:boolean;total:number;start:()=>void;className?:string}> =
