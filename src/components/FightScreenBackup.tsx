...
interface TaskTimer {
  taskId: string;
  estimatedTime: number;
  timeRemaining: number; // negative values = overtime
...
const FightScreen: React.FC = () => {
  ...
  // Audio setup
  useEffect(() => {
    const sounds = ['punch', 'grunt', 'victory', 'warning'];
...
const updatedTaskTimers = prev.taskTimers.map((timer, index) => {
  ...
  const taskElapsed = rawTaskElapsed * TEST_MODE_SPEED_MULTIPLIER;
  const totalTaskTime = timer.estimatedTime * 60 * 1000;
  const taskRemaining = totalTaskTime - taskElapsed; // Can be negative (overtime)
  const taskRemainingSeconds = Math.ceil(taskRemaining / 1000);

// 30-second warning (only once, using range to avoid duplicates)
if (taskRemainingSeconds <= 30 && taskRemainingSeconds > 25 && !timer.hasPlayedWarning) {
  console.log(`⚠️ 30-second warning for task: ${timer.taskId}`);
  playSound('warning'); // Custom warning sound
  return {
    ...timer,
    timeRemaining: taskRemainingSeconds,
    hasPlayedWarning: true
  };
}

// Timer hits zero - apply damage immediately (only once)
if (taskRemainingSeconds <= 0 && timer.timeRemaining > 0 && !timer.damageApplied) {
  console.log(`💥 Task timer expired: ${timer.taskId} - Applying damage`);
  const damage = timer.estimatedTime * 4;
  playSound('grunt'); // Damage sound
  ...
  return {
    ...timer,
    timeRemaining: taskRemainingSeconds // Negative = overtime
  };
});
...
return {
  ...
  taskTimers: updatedTaskTimersWithDamage, // Use the damage-cleared version
  ...
  // If negative (overtime), convert to positive for display
  ...
  taskTimer.timeRemaining <= 0 ? 'text-red-400 animate-pulse' : 
  taskTimer.timeRemaining <= 30 ? 'text-yellow-400 animate-pulse' : 'text-cyan-400'
}`}>
