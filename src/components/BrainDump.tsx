  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="text-neonRed hover:text-neonRed/80 text-xs opacity-60 hover:opacity-100 transition-opacity flex-shrink-0 px-1 py-1 rounded hover:bg-neonRed/20"
        title="Delete task"
      >
        ✕
      </button>
...
const DroppableRound: React.FC<{ 
  ...
  const getRoundColor = (status: string) => {
    switch (status) {
      case 'optimal': return 'border-neonYel bg-neonYel/10';
      case 'overfilled': return 'border-neonRed bg-neonRed/10';
  ...
  return (
    <div 
      ...
      className={`border rounded-lg p-3 min-h-[80px] transition-all ${getRoundColor(status)} ${
        isOver ? 'ring-2 ring-neonYel bg-neonYel/20 border-neonYel' : ''
      }`}
    >
        ...
        <div className="bg-black/30 rounded-lg p-4 border border-crtBlue mb-6">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-neonYel text-black px-4 py-2 rounded font-mono text-sm font-bold hover:bg-neonYel/80 disabled:opacity-50 flex items-center gap-2 transition-colors"
            >
              <Plus size={14} />
              ADD
            </button>
          ...
          <div className="p-4 border-t border-crtBlue/50 flex items-center justify-between">
            ...
            <button
              ...
              className={`font-mono text-sm px-6 py-2 rounded border transition-all font-bold ${
                canStartTournament
                  ? isFullTournament
                    ? 'bg-neonRed border-neonRed text-white hover:bg-neonRed/80'
